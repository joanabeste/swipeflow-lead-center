-- 101: merge_lead Datenverlust-Fix.
--
-- Bisher (100): bei einem unique_violation beim Umhängen der Kind-Zeilen wurde
-- pauschal `DELETE FROM tbl WHERE col = loser` ausgeführt — das löschte ALLE
-- Loser-Zeilen der Tabelle, nicht nur die kollidierende. Nicht-kollidierende
-- Kind-Zeilen (z.B. lead_calls) gingen dabei unwiderruflich verloren.
--
-- Jetzt: Fast-Path bleibt das Bulk-UPDATE. Schlägt es wegen eines Unique-
-- Konflikts fehl, fällt die Funktion auf eine Pro-Zeilen-Behandlung über `ctid`
-- zurück: jede Loser-Zeile wird einzeln umgehängt; nur die tatsächlich
-- kollidierende Zeile wird gelöscht. So bleiben alle umhängbaren Daten erhalten.

...
/**
 * Pro-Nutzer-Einstellung für das Qualifizierungs-Cockpit speichern: ob Taste „1"
 * sofort qualifiziert (→ CRM) und in welchen CRM-Status grün-qualifizierte
 * Leads wandern. Gespeichert pro Nutzer (Key `qualify_hotkey:<userId>`), sodass
 * jede:r ein eigenes Verhalten hat. Vertrieb-Berechtigung genügt.
 */
...
> {
  ...
  const { targetStatusId } = await getQualifyHotkeySettings(ctx.user.id);
  const db = createServiceClient();

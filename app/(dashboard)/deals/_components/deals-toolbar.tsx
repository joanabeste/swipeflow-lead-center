...
export function DealsToolbar({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-gray-200 bg-white p-3 dark:border-[#2c2c2e]/50 dark:bg-[#1c1c1e]">
      ...
      {/* Monats-Scope: gewonnene/verlorene Deals nur aus diesem Monat; offene immer. */}
      <select
        value={month}
        onChange={(e) => onMonthChange(e.target.value)}
        title="Abschluss-Monat (gewonnene/verlorene Deals)"
        className="rounded-md border border-gray-200 bg-white px-2.5 py-1 text-xs capitalize dark:border-[#2c2c2e] dark:bg-[#232325]"
      >
        {monthOptions.map((m) => (
